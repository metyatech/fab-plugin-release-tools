...
test('format bootstrap selects the visible button-only Fab chooser control before Next', async () => {
  const { result, fixture } = await scenario({ state: { productFormats: [], formatChoiceNeedsNext: true, formatChoiceButtonOnly: true }, mode: 'save', saveDraftAuthorized: true });
  assert.equal(result.result, 'PASS');
  assert.equal(result.formatBootstrapCreated, true);
  assert.equal(fixture.state.productFormats.filter((format) => format.name === 'Unreal Engine').length, 1);
  assert.equal(fixture.mutations.filter((item) => item.pathname === '/api/create-format').length, 1);
...
test('format bootstrap fills the observed Fab placeholder-based version form', async () => {
  const { result, fixture } = await scenario({ state: { productFormats: [], formatChoiceNeedsNext: true, realObservedVersionForm: true }, mode: 'save', saveDraftAuthorized: true });
  assert.equal(result.result, 'PASS');
  assert.equal(result.formatBootstrapCreated, true);
  assert.equal(fixture.state.productFormats.filter((format) => format.name === 'Unreal Engine').length, 1);
  assert.equal(fixture.mutations.filter((item) => item.pathname === '/api/create-format').length, 1);
...
test('format bootstrap waits for the delayed version Confirm control', async () => {
  const { result, fixture } = await scenario({
    state: { productFormats: [], formatChoiceNeedsNext: true, realObservedVersionForm: true, confirmDelayMs: 250 },
    mode: 'save',
    saveDraftAuthorized: true,
  });
  assert.equal(result.result, 'PASS');
