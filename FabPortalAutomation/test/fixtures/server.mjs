...
function pageMarkup(state, listingId) {
  ...
  const versionForm = state.formatChoiceNeedsNext ? `<h2>Add Unreal Engine version</h2><label>Version title *<input aria-label="Version title *"></label><label>Project file link *<input aria-label="Project file link *"></label>${state.customVersionControls ? `<input role="combobox" aria-label="Supported engine version *" placeholder="Search or select engine versions"><div role="option" aria-label="${html(state.engineVersions[0] ?? '5.8')}">${html(state.engineVersions[0] ?? '5.8')}</div><input role="combobox" aria-label="Supported target platforms *" placeholder="Search or select target platforms"><div role="option" aria-label="${html(state.platforms?.[0] ?? 'Win64')}">${html(state.platforms?.[0] ?? 'Win64')}</div>` : `<label>Supported engine version *<select aria-label="Supported engine version *"><option>${html(state.engineVersions[0] ?? '5.8')}</option></select></label><label>Supported target platforms *<select aria-label="Supported target platforms *"><option>${html(state.platforms?.[0] ?? 'Win64')}</option></select></label>`}` : '';
